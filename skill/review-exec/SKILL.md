...
`git diff`コマンド等を実行し、レビュー対象となる変更差分を確認する。
ソースコードが変更後の状態になっていない場合は`git switch`コマンド等で切り替える。

確認した結果をもとに、レビュー報告書のテンプレートを作成する。レビュー計画書がある場合はその末尾に追記する。レビュー計画書が無ければ `tmp/review-yyyymmdd-{ブランチ名}.md`に新規作成する。
